...
import { changeSearchFilters, toggleResourceMap, sResMap, hResMap } from 'actions/uiActions';
...
class UnconnectedSearchPage extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.showMap) actions.toggleMap();
    if (!searchDone) {
          ...
          <div className="app-SearchPage__content">
            {(searchDone || isFetchingSearchResults) &&
              <SearchResults
                ...
                selectedUnitId={selectedUnitId}
                showMap={showMap}
                sResMap={actions.sResMap}
                toggleMap={actions.toggleResourceMap}
              />
            }
          </div>
